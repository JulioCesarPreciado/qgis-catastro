...
class LoginMaster:
    ...
    def initGui(self):
        ...
        self.actionRusticos.setEnabled(False)  ## False
        ...
        self.actionSalir = QtWidgets.QAction(self.iface.mainWindow())        
        self.actionSalir.setObjectName("actionSalir")
        self.actionSalir.triggered.connect(self.salir)
        self.iface.addToolBarIcon(self.actionSalir)
        self.actionSalir.setText(self.tr(u'Salir'))
        self.actionSalir.setEnabled(False) 
        self.actions.append(self.actionSalir)
    ...
    def logins(self):
        ...
        if self.first_start == True:
            ...
            if result:
                ...
                cur.execute( "SELECT * FROM usuario WHERE usuario = '" + nombre + "' AND password_carto = '" + password + "'" )        
                if cur.rowcount != 0:
                    ventanaReg=None
                    self.first_start = False
                    ...
                    self.actionSalir.setEnabled(True) 
                    iface.messageBar().pushMessage("Correcto", "Bienvenido", level=Qgis.Success)
                    
                    ######################### ecw
                    #pathc="C:\CARTOGRAFIA 2021\Nogales\Entregables\OrtofotoDigital\ECW\nogales.ecw"
                    pathc = os.path.join(os.path.dirname(__file__), "nogales.ecw")
                    print(pathc)
                    fileInfo = QFileInfo(pathc)
                    path = fileInfo.filePath()


                    rlayer_ecw = QgsRasterLayer(path, 'Nogales')
                    if not rlayer_ecw.isValid():
                        iface.messageBar().pushMessage("Error", "Layer failed to load!", level=Qgis.Success)
                    QgsProject.instance().addMapLayer(rlayer_ecw)
                    ...
                    urlManzana = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=manzanas&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    ...
                    #urlPredios = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=muestra_predios_v2&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    urlPredios = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=predios&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    ...
                    #SUBPREDIOS 
                    urlSubPredios = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=subpredios&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                   
                    rlayerSubPredios = QgsRasterLayer(urlSubPredios, 'SubPredios', 'wms')
                    if not rlayerSubPredios.isValid():
                        iface.messageBar().pushMessage("Error", "Layer failed to load!", level=Qgis.Success)
                        
                    QgsProject.instance().addMapLayer(rlayerSubPredios)



                    #urlZonas = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=zonas&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    urlZonas = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=zonas&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    ...
                    #urlBandas = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=bandas_v1&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    urlBandas = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=bandas&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    ...
                    #urlConstrucciones = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=muestra_construcciones_v2&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    urlConstrucciones = "crs=EPSG:32612&dpiMode=7&format=image/png&layers=construcciones&styles&url=http://192.168.100.246:9090/geoserver/nogales_catastro/wms?version%3D1.1.0%26layers%3Dnogales_catastro"
                    ...
                    QgsProject.instance().layerTreeRoot().findLayer(rlayerConstrucciones.id()).setItemVisibilityChecked(False)
                    
                    
                    #qgis.utils.iface.setActiveLayer(rlayerPredios)
                    #qgis.utils.iface.zoomToActiveLayer()
                    canvas = iface.mapCanvas()
                    extent = rlayerPredios.extent()
                    ...
                    iface.messageBar().pushMessage("Usuario o contraseña no validos", level=Qgis.Success)
        else:
            iface.messageBar().pushMessage("ya tiene una sesion iniciada!", level=Qgis.Success)
    ...
    def buscar(self):
        ...
        if self.first_start2 == True:
            ...
            if result:
                ...
                lineedit_textsplit=lineedit_text[0:4]
    ...
    def tramites(self):
        ...
        if self.first_start3 == True:
            ...
            self.dlg3.comboBox.addItems(["Seleccionar", "ACTUALIZAR AVALUO", "ACTUALIZAR AVALUO RUSTICO","EXCEDENCIA URBANA","EXCEDENCIA RUSTICA" ,"FUSION URBANA","FUSION RUSTICA", "SUBDIVISION URBANA", "SUBDIVISION RUSTICA","ACTUALIZAR AVALUO GLOBAL DE CONDOMINIO","SEMBRAR CONDOMINIO"])
            ...
            if (gConnMySql):
                ...
                def field_select():
                    ...
                    ids_gestion = []
                    #usuario pruebas Miguelpgm
                    #cur.execute( "SELECT id_gestion FROM gestion WHERE solicitud = '" + sel + "' AND usuario_control = '" + usuario + "' AND estatus = 'EN TRAMITE' AND (geometrias_listas = 'N' OR  geometrias_listas IS NULL)")
                    #cur.execute( "SELECT id_gestion FROM gestion WHERE solicitud = '" + sel + "' AND usuario_control = '" + usuario + "' AND estatus = 'EN TRAMITE'") # AND (geometrias_listas = 'N' OR  geometrias_listas IS NULL)")
                    cur.execute( "SELECT replace(convert(varchar(10), id_gestion) + '->'+ catastral,' ','')  as id_gestion FROM gestion WHERE solicitud = '" + sel + "' AND usuario_control = '" + usuario + "' AND estatus = 'EN TRAMITE'") # AND (geometrias_listas = 'N' OR  geometrias_listas IS NULL)")
             
                    for id_gestion in cur.fetchall() :
                        dato_gestion = str(id_gestion)
                        print(dato_gestion)
                        ids_gestion.append(dato_gestion[2:-4])
                    ids_gestion.sort()
            ...
            if result:
                ...
                if (gConnMySql):
                    ...
                    cur = gConnMySql.cursor()
                    sel = str(self.dlg3.comboBox.currentText())
                    selgesX = str(self.dlg3.comboBox_2.currentText())
                    #nuevo para dejar solo gestion
                    selges=selgesX[0: selgesX.index('-')]
                    ...
                    if (sel=="FUSION URBANA" or sel=="FUSION RUSTICA") :
                        ...
                        cur.execute( "SELECT catastral FROM predios_a_procesar WHERE id_gestion = '" + selges + "' ORDER BY catastral DESC" )
                        for catastral in cur.fetchall() :
                        ...
                        print('--------- ESTO ES TRAMITE ----------')
                        print(clv_catastral)
                        row = cur.fetchone()
                        
                    else:
                        cur.execute( "SELECT catastral, unidades FROM gestion WHERE id_gestion = '" + selges + "' ORDER BY catastral DESC" )
                        for catastral in cur.fetchall() :
                ...
                con = fnConexionServidorPg()
                cur = con.cursor()  
                cur.execute("select catastral from nogales_catastro.predios where catastral='"+clv_catastral+"'")
                if cur.fetchall():
                    print("se encuentra en BD")
                    encuentraPredio=1
                else:
                    print("no encuentra BD")    
                ...
                print (exists)
                con.commit()
                if encuentraPredio==0:
                    iface.messageBar().pushMessage(" Predio no Vinculado", level=Qgis.Success)
                else:
                    if exists==False:
                            ...
                            if sel=="FUSION URBANA" or sel=="FUSION RUSTICA":
                                    ...
                                    print  ('Lista de claves antes del for: ' , listaclaves)
                                    claves = ''
                                    contador = 0
                                    ...
                                    #Delete from nogales_catastro.construcciones c where c.gid in (SELECT c.gid from nogales_catastro.construcciones c, nogales_catastro.predios p where ST_intersects(p.geom,ST_Centroid(c.geom)) and p.catastral = '"+clv_catastral+"')")
                                    contador = contador +1
                                    print (contador) 
                                    selectconstucciones = 'INSERT INTO nogales_catastro.construcciones_gestion_'+selges+' (fid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT gid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom from nogales_catastro.construcciones where ' + claves + ' order by catastral )'
                                    selectconstruccionesh = 'INSERT INTO historico_nogales_catastro.historico_construcciones_geo (fid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT gid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom from nogales_catastro.construcciones where ' + claves + ' order by catastral )'
                                    
                                    limpiaconstucciones = 'DELETE FROM nogales_catastro.construcciones_gestion_'+selges+' d WHERE  EXISTS (SELECT FROM nogales_catastro.construcciones_gestion_'+selges+' WHERE  fid = d.fid AND    gid < d.gid)'


                                    print (selectconstucciones)
                                    print (limpiaconstucciones)
                                    ...
                                    try:
                                        ...
                                        cur.execute("Delete FROM nogales_catastro.construcciones_gestion_"+selges+";")
                                        cur.execute(select)
                                        cur.execute(selecth)
                                        cur.execute(selectconstucciones)
                                        cur.execute(selectconstruccionesh)
                                        cur.execute(limpiaconstucciones)
                                        con.commit()
                                        print ('Tabla creada')
                                    except psycopg2.DatabaseError as e:
                                        print(f'Error {e}')
                    ...
                    for style in styles.getFeatures():
                        if style['stylename'] == 'predios_tramites':
                    ...
                    styleqml = ""
                    for style in styles.getFeatures():
                        if style['stylename'] == 'construcciones_tramites':
                            styleqml = style['styleqml']

                    styledoc = QDomDocument()
                    styledoc.setContent(styleqml)
                    layerConst.importNamedStyle(styledoc)
                    #######
                    ...
                    print('Tramite seleccionado:' +  selTramite)
                    ...
                    if selTramite == "SUBDIVISION RUSTICA":
                        ...
                        self.actionVunitario.setEnabled(False)
                        self.actionRusticos.setEnabled(True)
                    if selTramite == "ACTUALIZAR AVALUO RUSTICO":
                        print('ENTRO A: ACTUALIZAR AVALUO RUSTICO')
                        self.actionGeneraclv.setEnabled(False)
                        self.actionbtnconstrucciones.setEnabled(False)
                        self.actionVunitario.setEnabled(False)
                        self.actionRusticos.setEnabled(True)
            ...
            def canvasReleaseEvent(self, event):
                ...
                try:
                    ...
                    if clavecat=="DUPLICADOS O ERROR":
                        iface.messageBar().pushMessage("Seleccione un predio", level=Qgis.Success)
                    else:
        ...
        class SendPointToolCoordinates(QgsMapTool):
            ...
            def canvasReleaseEvent(self, event):
                ...
                if clv_gid_predio == "DUPLICADOS O ERROR":
                    iface.messageBar().pushMessage("seleccione unpredio", level=Qgis.Success)
                else:
                    print("entra gid")
                        ...
                        if len(clv_gid_predio)>0:
                            ...
                            if selTramite == "SUBDIVISION URBANA":
                                if ban_catastral_gestion==0:
                                    iface.messageBar().pushMessage( "Al menos un predio deve contar con la clave catastral de origen", level=Qgis.Success)
                                    ventanapredios=None
                                elif ban_catastral_gestion>1: 
                                    iface.messageBar().pushMessage("Dos predios no pueden tener la misma clave catastral", level=Qgis.Success)
                                    ventanapredios=None
                                elif  layer.featureCount() > unidades:
                                    iface.messageBar().pushMessage("No puede tener mas de "+str(unidades)+" predios", level=Qgis.Success)
                            ...
                            if ventanapredios is not None:
                                    ...
                                    if result:
                                        ...
                                        if not valorUG:
                                            ...
                                            #####################3
                                            if ban_error==1:
                                                iface.messageBar().pushMessage("El predio no se guardo, hay construcciones sin guardar", level=Qgis.Success) 
                                                ...
                                                try:
                                                    ...
                                                    if clv_catastral_predio==clv_catastral:
                                                        ...
                                                        cur.execute("Update nogales_catastro.predio_gestion_"+clv_gestion+" SET clv_mzn='"+clv_mzn+"', catastral='"+clv_catastral_predio+"', zona_h='"+zonaH+"' ,valor_zona="+str(zona)+", valor_band="+str(banda_max)+", id_gestion="+str(clv_gestion)+", area_terre="+str(areaterreno)+",usuario='"+usuario+"' , condominio="+str(condominio)+",factori="+str(fsctorI)+",vandamax="+str(valoresZb)+",geom=ST_GeomFromText('"+str(geom)+"', 32612), ban_cat=2 where gid ="+str(clv_gid_predio)+"")
                                                        con.commit()
                                                    else:
                ...
                if clv_gid_predio == "DUPLICADOS O ERROR":
                    ...
                    if len(clv_gid_predio)>0:
                        ...
                        if layer.featureCount() <= 1:
                            iface.messageBar().pushMessage("*Debe subdividir el predio antes de generar la nueva clave catastral", level=Qgis.Success)
                        elif  layer.featureCount() > unidades:
                            iface.messageBar().pushMessage("No puede tener mas de "+str(unidades)+" predios", level=Qgis.Success)
                            ...
                            # See if OK was pressed
                            if result:
                                print ("VValor ui: " ,str(ui.mLineEdit.text()))
                                datosGenclv=datosClv(ui)  #datosGenclv=datosClv(ui)  
                                print(datosGenclv)
                                print ('datosGen con 0')
                                
                                #print (str(datosGenclv[0]))
                                #nuevaClave=datosGenclv[0]  Así estaba originalmente 19 Oct 2021
                                nuevaClave=str(ui.mLineEdit.text()) # Cambio 20 / 10 /2021
                                nuevaClaveMzn = (str(ui.mLineEdit.text())[5:9])
                                print ("Clave Manzana:", nuevaClaveMzn )
                                print("claves iguales",num_clvs_igiales)
                                if num_clvs_igiales==1 and nuevaClave==clv_catastral:  # if num_clvs_igiales==1 and nuevaClave!=clv_catastral:
                                    print(num_clvs_igiales)
                                    print(nuevaClave)

                                    iface.messageBar().pushMessage("Por lo menos un predio debe concervar la clave catastral original", level=Qgis.Success)
                                else:
                                    ...
                                    try:
                                        ...
                                        if selTramite=="SUBDIVISION RUSTICA":
                                                ...
                                                print("Update nogales_catastro.predio_gestion_"+str(clv_gestion)+" SET catastral='"+str(nuevaClave)+"', ban_cat=1 where gid="+str(clv_gid_predio))
                                                cur.execute("Update nogales_catastro.predio_gestion_"+str(clv_gestion)+" SET catastral='"+str(nuevaClave)+"', ban_cat=1 where gid="+str(clv_gid_predio))
                                        con.commit()
                                        #print ('Tabla creada')
...
#### Predios Rusticos
                ...
                if clv_gid_predio == "DUPLICADOS O ERROR":
                    ...
                    if len(clv_gid_predio)>0:
                        ...
                        if layer.featureCount() <= 0 :     
                            iface.messageBar().pushMessage("Debe exitir la geometría", level=Qgis.Success) 
                        else:    
                            num_clvs_igiales=0
                            for f in layer.getFeatures():
    ...
    def finalizarTramite(self):
        try:
                iface.messageBar().pushMessage( "Inicia Finalizar", level=Qgis.Success)
                global selTramite,usuario, clv_gestion , clv_catastral
                print(clv_catastral)
                registry = QgsProject.instance()
                ...
                if curP.fetchall():
                    ...
                    iface.messageBar().pushMessage("faltan predios por guardar", level=Qgis.Success)
                else:
                    iface.messageBar().pushMessage( "entra al else", level=Qgis.Success)
                    print("select  catastral, count(catastral) from nogales_catastro.predio_gestion_"+clv_gestion+" group by catastral having count(catastral)>1 ")
                    ...
                    if curP.fetchall():
                        ...
                        iface.messageBar().pushMessage("Clave catastral repetida", level=Qgis.Success)
                    else:
                        print("Antes de if de subdivision")
                        iface.messageBar().pushMessage( "Antes de if de subdivisione", level=Qgis.Success)
                        if selTramite=="SUBDIVISION RUSTICA" or selTramite=="FUSION RUSTICA" or selTramite== "ACTUALIZAR AVALUO RUSTICO":
                            print("Entro a la validación de Subdivisin rustica")
                            layer = registry.mapLayersByName( 'predio_origen' )[0]
                        ...
                        #construcciones
                        #
                        iface.messageBar().pushMessage( "Construcciones entro", level=Qgis.Success)
                        ban_error=0
                        lyrconst = registry.mapLayersByName( 'construcciones_origen' )[0]
                        iface.messageBar().pushMessage( "Paso este layer", level=Qgis.Success)
                        ### borrar construcciones  en sql
                        querydel="delete from dbo.construcciones_en_proceso where catastral = '"+clv_catastral+"' and id_gestion = "+str(clv_gestion) 
                        ...
                        try:
                            ...
                            print("error borrar const")
                        
                        print(lyrconst.getFeatures())
                        i = 0
                        for feature in lyrconst.getFeatures():
                            i = i + 1
                            print(feature["matriz1"])
                            
                            areaConst=feature["area_const"] * feature["niveles"]
                            valor_bloque_const= feature["vuc"] * areaConst
                            #querydel="delete from construcciones_en_proceso where catastral = "+feature["catastral"]+" and id_gestion = "+str(feature["id_gestion"])  
                            queryin = "INSERT INTO  construcciones_en_proceso (area_construccion, niveles, bloque, matriz, valor_bloque_const, vuc,id_gestion,capturo, catastral, objectid) values ("+str(areaConst)+","+str(feature["niveles"])+"," +str(feature["bloque"])+",'"+feature["matriz1"]+"',"+str(valor_bloque_const)+","+str(feature["vuc"])+","+str(clv_gestion)+",'"+feature["usuario"]+"','"+feature["catastral"]+"',"+str(feature["fid"])+")"
                            #print(querydel)
                            print(queryin)
                            try:
                                con = fnConexionServidor()
                                ...
                                ban_error=1
                                print("error insert") 
                        print("----------------- Esto es la cantidad de registros en el layer : " + str(i))
                        #Borrar en la capa de construcciones de produccion cuando se eliminen de las de origen
                        if i == 0:
                            #Borrar construcciones de la capa de producción cuando no existanla capa de trabajo 
                            print("Borrar construcciones")
                            try:
                                #borrar construcciones en postgres  interseccion del fusionado (seleccion espacial  contain)
                                #curP.execute("delete from nogales_catastro.construcciones where catastral = '"+clv_catastral+"'  ")
                                curP.execute("Delete from nogales_catastro.construcciones c where c.gid in (SELECT c.gid from nogales_catastro.construcciones c, nogales_catastro.predios p where ST_intersects(p.geom,ST_Centroid(c.geom)) and p.catastral = '"+clv_catastral+"')")
                                #curP.execute("INSERT INTO nogales_catastro.construcciones( mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom FROM nogales_catastro.construcciones_gestion_"+clv_gestion+" where catastral='" +str(clv_catastral)+ "' order by catastral)")
                            
                            except psycopg2.DatabaseError as e:
                                print("Construcciones de capa en produccion")
                                ban_error=1
                                print(f'Error {e}')
                        #Borrar contrucciones
                        if ban_error==0 and i >= 1:    
                            try:
                                #borrar construcciones en postgres
                                
                                # En produccion se cambiara por eliminación de construcciones intersecatadas con el predio gestion
                                #curP.execute("delete from nogales_catastro.construcciones where catastral = '"+clv_catastral+"'  ")
                                
                                # De prueba
                                #select c.gid from nogales_catastro.construcciones c, nogales_catastro.predio_gestion_189261 p where  ST_intersects(st_centroid(c.geom),p.geom)
                                #delete from nogales_catastro.construcciones a where a.gid in (select c.gid from nogales_catastro.construcciones c, nogales_catastro.predio_gestion_189261 p where  ST_intersects(st_centroid(c.geom),p.geom))

                                qborraconst ="delete from nogales_catastro.construcciones a where a.gid in (select c.gid from nogales_catastro.construcciones c, nogales_catastro.predio_gestion_"+clv_gestion+" p where  ST_intersects(st_centroid(c.geom),p.geom))"
                                print (qborraconst)
                                print ("INSERT INTO nogales_catastro.construcciones( mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom FROM nogales_catastro.construcciones_gestion_"+clv_gestion+" where catastral='" +str(clv_catastral)+ "' order by catastral)")
                                curP.execute(qborraconst)
                                curP.execute("INSERT INTO nogales_catastro.construcciones( mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom FROM nogales_catastro.construcciones_gestion_"+clv_gestion+" where catastral='" +str(clv_catastral)+ "' order by catastral)")
                        ...
                        if ban_error==1:
                            ...
                            lyrpredio = registry.mapLayersByName( 'predio_origen' )[0]
                            for feature in lyrpredio.getFeatures():
                                #####################
                                try:  #ACTUALIZAR AVALUO RUSTICO  EXCEDENCIA RUSTICA FUSION RUSTICA SUBDIVISION RUSTICA
                                    print (selTramite)
                                    #if selTramite != 'ACTUALIZAR AVALUO RUSTICO' or selTramite != 'EXCEDENCIA RUSTICA' or  selTramite != 'FUSION RUSTICA' or selTramite != 'SUBDIVISION RUSTICA':
                                    if (selTramite.find('RUSTICO') == -1):
                                        print ('NO es un tramite Rustico')
                                        print("SELECT zona_h, valor_zona, valor_band, area, gid FROM nogales_catastro.predio_vu_"+clv_gestion+"_"+clv_catastral)
                                        curP.execute("SELECT zona_h, valor_zona, valor_band, area, gid FROM nogales_catastro.predio_vu_"+clv_gestion+"_"+clv_catastral+" ")
                                        data=curP.fetchall()
                                         ### guardar en sql
                                        try:         
                                            ...
                                            for row in data: 
                                                ...
                                                if row[2] is None:
                                                    ...
                                                    valB=row[2]
                                                    print("*****sql tabla  valor unitario geo******")
                                            
                                                    querydel="delete from valor_unitario_geo where catastral = '"+feature["catastral"]+"' and id_gestion = "+str(clv_gestion)
                                                    print(querydel)
                                                    cursql.execute(querydel)
                                                    cursql.execute("insert into valor_unitario_geo ( id_gestion, catastral, zona,valorz, vbanda, area, valor_unitario_gral, factor_irregularidad, fecha_avaluo, usuario) values ("+str(clv_gestion)+",'"+feature["catastral"]+"','" +str(row[0])+"',"+str(valZ)+","+str(valB)+","+str(row[3])+","+str(feature["vandamax"])+","+str(feature["factori"])+",getdate(),'"+usuario+"')")
                                                    
                                                consql.commit()
                                        except:
                                            print("error guardar vu ")
                                                
                                except psycopg2.DatabaseError as e:
                                    print(f'Error {e}')
                                
                                ####################  Diferenciar Predios y Predios Rusticos
                                querydel="delete from predios_en_proceso where catastral = '"+feature["catastral"]+"' and id_gestion = "+str(clv_gestion)  
                                queryin = "insert into predios_en_proceso (catastral, area_terreno, id_gestion, objectid,capturo, valor_unitario, factor_irregularidad, zona_homogenea) values ('" +feature["catastral"] + "'," + str(feature["area_terre"]) + "," + str(clv_gestion) + "," + str(feature["fid"]) + ",'" + str(feature["usuario"]) + "'," + str(feature["vandamax"]) + "," + str(feature["factori"]) + ",'" + str(feature["zona_h"]) + "')"
                                print(queryin)
                                try:
                                    con = fnConexionServidor()
                                    cur = con.cursor() 
                                    ...
                                    ban_error=1
                                    iface.messageBar().pushMessage("El predio no se guardo", level=Qgis.Success) 
                                                    
                                    print("error insert")
                                                            
                                fnConexionCerrar(con)
                                print("Valor de ban_error antes de actualizar predios")
                                print(ban_error)
                                if ban_error==0:
                                    try:
                                        #borrar construcciones en postgres
                                        #if feature["catastral"]==clv_catastral:
                                        geom=feature.geometry().asWkt()
                                        #    print(clv_catastral)
                                        #    print("Entra Update de clave ",clv_gid_predio)
                                        #    curP.execute("Update nogales_catastro.predios SET  valor_zona="+str(feature["valor_zona"])+", valor_band="+str(feature["valor_band"])+", id_gestion="+str(clv_gestion)+", area_terre="+str(feature["area_terre"])+",usuario='"+feature["usuario"]+"' , condominio="+str(feature["condominio"])+",geom=ST_GeomFromText('"+str(geom)+"', 32612) where catastral ='"+str(feature["catastral"])+"'")
                                        #    #Eliminar los predios donde el centroide esten dentro de la clave catastral
                                        #else:
                                        #    print("Entra Insert de clave ",clv_gid_predio)
                                        #    curP.execute("insert into nogales_catastro.predios (claves, clv_mzn, catastral, zona_h, valor_zona, valor_band, ubicacion_, propie, id_gestion, area_terre, id_catastr, usuario, condominio, geom)(SELECT claves, clv_mzn, catastral, zona_h, valor_zona, valor_band, ubicacion_, propie, id_gestion, area_terre, id_catastr, usuario, condominio, geom from nogales_catastro.predios_gestion_"+clv_gestion+" where catastral='" +str(feature["catastral"])+"'") 
                                        print ("Delete from nogales_catastro.predios p where ST_intersects(ST_GeomFromText('"+str(geom)+"', 32612),ST_Centroid(p.geom))") 
                                        print ("insert into nogales_catastro.predios (claves, clv_mzn, catastral, zona_h, valor_zona, valor_band, ubicacion_, propie, id_gestion, area_terre, id_catastr, usuario, condominio, geom)(SELECT right(trim(catastral),3) as claves, clv_mzn, catastral, zona_h, valor_zona, valor_band, ubicacion_, propie, id_gestion, area_terre, id_catastr, usuario, condominio, geom from nogales_catastro.predio_gestion_"+clv_gestion+" where catastral='" +str(feature["catastral"])+"')") 
                                        curP.execute("Delete from nogales_catastro.predios p where ST_intersects(ST_GeomFromText('"+str(geom)+"', 32612),ST_Centroid(p.geom))") 
                                      
                                        curP.execute("insert into nogales_catastro.predios (claves, clv_mzn, catastral, zona_h, valor_zona, valor_band, ubicacion_, propie, id_gestion, area_terre, id_catastr, usuario, condominio, geom)(SELECT right(trim(catastral),3) as claves, clv_mzn, catastral, zona_h, valor_zona, valor_band, ubicacion_, propie, id_gestion, area_terre, id_catastr, usuario, condominio, geom from nogales_catastro.predio_gestion_"+clv_gestion+" where catastral='" +str(feature["catastral"])+"')") 
                                        
                                    except psycopg2.DatabaseError as e:
                                            print("error en update o insert predios")
                                            ban_error=1
                                            print(f'Error {e}')
                        ...
                        #geometrias listas
                        if ban_error==0: 
                            print("geometrias listas en sql")
                            ...
                            try:
                                ...
                                print("error update en update gestion sql")
                            ...
                            try:
                                ...
                                for feature in layer.getFeatures():
                                    ...
                                    curP.execute("DROP TABLE IF EXISTS nogales_catastro.predio_vu_"+clv_gestion+"_"+clvPred)
                            ...
                            self.actionGeneraclv.setEnabled(False)
                            self.iface.actionPan().trigger()
                            iface.messageBar().pushMessage("Trámite terminado", level=Qgis.Success)
    ...
    def salir(self):
        try:
                ...
                for layer in QgsProject.instance().mapLayers().values():
                        print(layer.name())
                        lyr_nombre=layer.name()
                        if 'predio_origen' in lyr_nombre:
                            layer = registry.mapLayersByName( 'predio_origen' )[0]
                            try:                    
                                for feature in layer.getFeatures():
                                    print(str(feature["catastral"]))
                                    clvPred=str(feature["catastral"])
                                    curP.execute("select exists(select relname from pg_class where relname='predio_vu_"+clv_gestion+"_"+clvPred+"')")
                                    exists = curP.fetchone()[0]
                                    if exists==True:
