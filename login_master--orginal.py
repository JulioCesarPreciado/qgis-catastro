        ...
        class SendPointToolCoordinates(QgsMapTool):
            ...
            def canvasReleaseEvent(self, event):
                ...
                if clv_gid == "DUPLICADOS O ERROR":
                    iface.messageBar().pushMessage("Error", "seleccione una construccion", level=Qgis.Success)
                else:
                    print("entra gid")
                    ...
                    try:
                        ...
                        if len(xy_catastral)>0:
                            ...
                            if result:
                                ...
                                #     cur.execute("INSERT INTO nogales_catastro.historico_construcciones_geo(fid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT gid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom FROM nogales_catastro.construcciones where gid=" +str(fid)+ " order by catastral)")
                                #     con.commit()
                                #     #print ('Tabla creada')
                                # except psycopg2.DatabaseError as e:
                                #     print(f'Error {e}')
                                #updateglobal clv_gestion
                                global usuario
                    ...
                    # ui = Ui_Dialog()
                    # ui.setupUi(self.dialog)
                    # ui.mLineEdit.setText(str(listaclaves[0]))
                    # self.dialog.show()
                        ...
                        if len(clv_gid_predio)>0:
                            ...
                            if ventanapredios is not None:
                                    ...
                                    if result:
                                        ...
                                        #global  condominio, sector, zona, valorzona, valorbanda
                                        #areaterreno=selection[0].geometry().area()
                                        #poliqg=selection[0].geometry()
                                        #featuress = layer.selectedFeatures()[0]
                                        #gid=featuress["fid"]
                                        ...
                                        #
                                        if not valorUG:
                                            return iface.messageBar().pushMessage("Error", "falta calcular valor unitario", level=Qgis.Success)
                                        else:
                                            try:
                                                con = fnConexionServidorPg()
                                            ...
                                            #
                                            ### llenar tabla valor unitario_geo sql
                                            try:
                                                con = fnConexionServidorPg()
                                                    
                                                cur = con.cursor()
                                                    
                                                cur.execute("SELECT zona_h, valor_zona, valor_band, area, gid FROM nogales_catastro.predio_vu_"+clv_gestion+"_"+clv_catastral+" ")
                                                data=cur.fetchall()
                                                ### guardar en sql
                                                try:
                                                    ...
                                                    for row in data: 
                                                        ...
                                                        if row[2] is None:
                                                            ...
                                                            valB=row[2]
                                                        print("*****sql tabla******")
                                                        print("insert into valor_unitario_geo (id_valor_unitario_geo, id_gestion, catastral, zona,valorz, vbanda, area, valor_unitario_gral, factor_irregularidad, fecha_avaluo, usuario) values ("+str(row[4])+","+str(clv_gestion)+",'"+clv_catastral_predio+"','" +str(row[0])+"',"+str(valZ)+","+str(valB)+","+str(row[3])+","+str(valorUG)+","+str(fsctorI)+",getdate(),'"+usuario+"')")
                                                        cursql.execute("insert into dbo.valor_unitario_geo ( id_gestion, catastral, zona,valorz, vbanda, area, valor_unitario_gral, factor_irregularidad, fecha_avaluo, usuario) values ("+str(clv_gestion)+",'"+clv_catastral_predio+"','" +str(row[0])+"',"+str(valZ)+","+str(valB)+","+str(row[3])+","+str(valorUG)+","+str(fsctorI)+",getdate(),'"+usuario+"')")
                                            ...
                                            querydel="delete from dbo.construcciones_en_proceso where catastral = '"+clv_catastral_predio+"' and id_gestion = "+str(clv_gestion)  
                                            print(querydel)
                                            try:
                                            ...
                                            for feature in lyrconst.getFeatures():
                                                ...
                                                if feature["fid"]=='NULL':
                                                    iface.messageBar().pushMessage("Error", "Hay construcciones sin guardar", level=Qgis.Success) 
                                                    ban_error=1
                                                    ...
                                                    ban_error=1
                                                    iface.messageBar().pushMessage("Error", "Hay construcciones sin guardar2", level=Qgis.Success)
                                                    self.dialog.close()
                                                    ...
                                                    queryin = "insert into dbo.construcciones_en_proceso (area_construccion, niveles, bloque, matriz, id_gestion, catastral, objectid) values ("+str(areaConst)+","+str(feature["niveles"])+"," +str(bloquepredio)+",'"+feature["matriz1"]+"',"+str(clv_gestion)+",'"+feature["catastral"]+"',"+str(feature["fid"])+")"
                                                    #print(querydel)
                                                    print(queryin)
                                                    try:
                                                        ...
                                                        print("error")
                                            if ban_error==1:
                                                iface.messageBar().pushMessage("Error", "El predio no se guardo por error en las construcciones", level=Qgis.Success) 
                                                print("error sale del loop")
                                            else:
                                                #guardar costrucciones en postgis
                                                querydelpredios="delete from nogales_catastro.construcciones where catastral = '"+clv_catastral_predio+"'"  
                                            
                                                try:
                                                    con = fnConexionServidorPg()
                                                    #con = psycopg2.connect(database='postgis_geo', user='postgres',password='taQ99Zm')
                                                    cur = con.cursor()
                                                    cur.execute(querydelpredios)     
                                                    cur.execute("INSERT INTO nogales_catastro.construcciones(gid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom) (SELECT fid, mslink, mapid, clavecat, matriz1, matriz2, tipoconst, edadconst, nivconst, area_pol, area_const, catastral, niveles, nivel, id_catastr, id_gestion, usuario, fecha_a_ed, bloque, geom FROM nogales_catastro.construcciones_gestion_"+clv_gestion+" where catastral='" +str(clv_catastral)+ "' order by catastral)")
                                                    con.commit()
                                                    #print ('Tabla creada')
                                                except psycopg2.DatabaseError as e:
                                                ...
                                                querydel="delete from predios_en_proceso where catastral = "+clv_catastral_predio+" and id_gestion = "+str(clv_gestion)  
                                                queryin = "insert into predios_en_proceso (catastral, area_terreno, id_gestion, objectid, valor_unitario, factor_irregularidad, zona_homogenea) values ('" +clv_catastral_predio + "'," + str(areaterreno) + "," + str(clv_gestion) + "," + str(gid) + "," + str(valoresZb) + "," + str(fsctorI) + ",'" + str(zonaH) + "')"
                                                print(queryin)
                                                try:
                                                    con = fnConexionServidor()
                                                    cur = con.cursor() 
                                                    ...
                                                    #symbol.setColor(QColor.fromRgb(80,207,48))
                                                    

                                                    layer.triggerRepaint()
                                                
                                                except:
                                                    iface.messageBar().pushMessage("Error", "El predio no se guardo por error en el predio", level=Qgis.Success) 
